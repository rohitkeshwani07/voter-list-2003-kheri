#!/usr/bin/env python3
"""
Extract all names from ECI PDF voter lists
"""
import subprocess
import os
import re
from pathlib import Path
import sys

def convert_pdf_to_images(pdf_path, output_dir):
    """Convert PDF pages to PNG images"""
    print(f"Converting {pdf_path}...", flush=True)
    cmd = [
        'pdftoppm',
        pdf_path,
        f'{output_dir}/page',
        '-png'
    ]
    # Suppress stderr warnings
    result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True)
    if result.returncode != 0:
        print(f"Error converting {pdf_path}", flush=True)
        return []

    # Get list of generated images
    images = sorted(Path(output_dir).glob('page-*.png'))
    return images

def ocr_image(image_path):
    """Run OCR on an image with Hindi+English support"""
    cmd = [
        'tesseract',
        str(image_path),
        'stdout',
        '-l', 'hin+eng'
    ]
    result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True)
    if result.returncode != 0:
        return ""
    return result.stdout

def extract_names_from_text(text):
    """Extract names from OCR text"""
    names = []
    lines = text.split('\n')

    for line in lines:
        # Skip empty lines and header lines
        line = line.strip()
        if not line:
            continue

        # Look for lines that appear to be voter entries
        # These typically have: number, name, relation, parent name, gender, age, ID
        # We'll extract any Devanagari text that looks like a name

        # Split by common delimiters
        parts = re.split(r'\s{2,}|\t|\|', line)

        for part in parts:
            part = part.strip()
            # Check if part contains Devanagari characters
            if re.search(r'[\u0900-\u097F]', part):
                # Remove numbers, special chars, keep Devanagari text
                cleaned = re.sub(r'[0-9\(\)\[\]\/\\]', ' ', part)
                cleaned = re.sub(r'\s+', ' ', cleaned).strip()

                # Only keep if it has substantial Devanagari content
                if len(cleaned) >= 2 and re.search(r'[\u0900-\u097F]', cleaned):
                    names.append(cleaned)

    return names

def process_pdf(pdf_path, temp_dir):
    """Process a single PDF and extract all names"""
    print(f"\nProcessing: {pdf_path}", flush=True)

    # Convert PDF to images
    images = convert_pdf_to_images(pdf_path, temp_dir)
    if not images:
        print(f"No images generated for {pdf_path}", flush=True)
        return []

    all_names = []

    # OCR each image
    for i, image in enumerate(images, 1):
        print(f"  OCR page {i}/{len(images)}...", flush=True)
        text = ocr_image(image)
        names = extract_names_from_text(text)
        all_names.extend(names)

    print(f"  Extracted {len(all_names)} name entries from {len(images)} pages", flush=True)

    # Clean up images
    for image in images:
        image.unlink()

    return all_names

def main():
    pdf_dir = Path('eci_pdfs')
    temp_dir = Path('temp_ocr')
    temp_dir.mkdir(exist_ok=True)

    output_file = 'all_names.txt'

    # Get all PDFs
    pdf_files = sorted(pdf_dir.glob('*.pdf'))
    print(f"Found {len(pdf_files)} PDF files", flush=True)

    all_names = []

    # Process each PDF
    for i, pdf_file in enumerate(pdf_files, 1):
        print(f"\n{'='*60}", flush=True)
        print(f"PDF {i}/{len(pdf_files)}: {pdf_file.name}", flush=True)
        print(f"{'='*60}", flush=True)

        try:
            names = process_pdf(pdf_file, temp_dir)
            all_names.extend(names)

            # Save progress periodically (every 10 PDFs)
            if i % 10 == 0:
                print(f"\n--- Saving progress ({len(all_names)} names so far) ---")
                with open(output_file, 'w', encoding='utf-8') as f:
                    for name in all_names:
                        f.write(name + '\n')

        except Exception as e:
            print(f"Error processing {pdf_file}: {e}")
            continue

    # Save final results
    print(f"\n{'='*60}")
    print(f"Extraction complete!")
    print(f"Total names extracted: {len(all_names)}")
    print(f"Saving to {output_file}...")
    print(f"{'='*60}")

    with open(output_file, 'w', encoding='utf-8') as f:
        for name in all_names:
            f.write(name + '\n')

    # Clean up temp directory
    import shutil
    shutil.rmtree(temp_dir)

    print(f"\nDone! All names saved to {output_file}")

if __name__ == '__main__':
    main()
